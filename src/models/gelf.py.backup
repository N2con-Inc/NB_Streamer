...
class GELFMessage(BaseModel):
    ...
    def from_netbird_event(
        ...
        """
        Create GELF message from Netbird event data.
        ...
        # Prepare custom fields with NB_ prefix and tenant
        custom_fields = {}
        
        # Add tenant field
        custom_fields["_NB_tenant"] = tenant_id
        
        # Add all Netbird fields with NB_ prefix
        for key, value in event_data.items():
            if key not in ["timestamp", "level"]:  # Skip fields handled above
                # Convert nested objects and datetime to JSON strings
        ...
        # Prepare serializable event data for full_message
