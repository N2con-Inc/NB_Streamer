...
class AuthService:
    ...
    def __init__(self):
        ...
        self.bearer_security = HTTPBearer(auto_error=False) if self.auth_type == "bearer" else None
        self.basic_security = HTTPBasic(auto_error=False) if self.auth_type == "basic" else None
    ...
    async def _authenticate_basic(self, request: Request) -> bool:
        ...
        if not (self._secure_compare(username, config.auth_username) and 
                self._secure_compare(password, config.auth_password)):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
