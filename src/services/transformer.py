...
class TransformerService:
    ...
    async def transform_event(self, raw_event_data: Dict[str, Any]) -> GELFMessage:
        ...
        try:
            ...
            # We'll try to use the regular transformation but with a fallback short message
    ...
    def validate_event_structure(self, raw_event_data: Dict[str, Any]) -> bool:
        ...
        interesting_fields = [
            "type", "event_type", "timestamp", "user", "peer", 
            "network", "action", "level", "message", "id", "source"
        ]
        
        found_fields = [field for field in interesting_fields if field in raw_event_data]
        if found_fields:
